...
// Select the reference branch immediate based on the decoded instruction type.
// reflects that JAL, B, CJ and CB should be mutually exclusive.
// The selected immediate is used to calculate the expected branch target (PC + immediate).
...
// offset is negative". Assumes jal_taken/no_prediction_when_invalid/no_prediction_not_branch/
...
//At most one of the four branch-instruction type flags (J,
//B, CJ, CB) is active at a time when the fetch is valid

`ASSERT(branch_ins_type_onehot,
        `IMPLIES(fetch_valid_i,
                 $onehot0({f_is_jal, f_is_branch, f_is_cj, f_is_cb})))
