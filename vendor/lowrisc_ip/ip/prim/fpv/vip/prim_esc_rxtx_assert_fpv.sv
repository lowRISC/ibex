...
module prim_esc_rxtx_assert_fpv (
  ...
  `ASSUME_FPV(PingEnStaysAsserted0_M,
              ping_req_i |=> (ping_req_i && !ping_ok_o) or(ping_req_i && ping_ok_o ##1 $fell(
                  ping_req_i
              )),
  ...
  `ASSERT(PingRespCheck_A, !error_present [* 4] ##1 $rose(ping_req_i) |-> ##[0:4] ping_ok_o, clk_i,
          !rst_ni || error_present)

  // be more specific (i.e. use throughout)
  `ASSERT(EscRespCheck_A,
          ##1 esc_req_i |-> ##[0:1] prim_esc_rxtx_fpv.esc_rx_out.resp_p ##1
  ...
  `ASSERT(FsmLivenessSender_A,
          (prim_esc_rxtx_fpv.i_prim_esc_sender.state_q !=
