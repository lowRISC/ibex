...
module prim_alert_rxtx_async_assert_fpv (
  ...
  `ASSUME_FPV(PingEn_M,
              $rose(
                  ping_req_i
              ) |-> ping_req_i throughout (ping_ok_o || error_present) [-> 1] ##1 $fell(
                  ping_req_i
              ),
              clk_i, !rst_ni)
  ...
  `ASSERT(PingHs_A,
          ##1 $changed(
              prim_alert_rxtx_async_fpv.ping_pd
  ...
  `ASSERT(AlertPingOk_A,
          !(prim_alert_rxtx_async_fpv.i_prim_alert_sender.state_q inside {
      prim_alert_rxtx_async_fpv.i_prim_alert_sender.PingHsPhase1,
      prim_alert_rxtx_async_fpv.i_prim_alert_sender.PingHsPhase2}) && $rose(
              ping_req_i
          ) |-> ##[1:23] ping_ok_o,
          clk_i, !rst_ni || error_setreg_q)
  ...
  `ASSERT(FsmLivenessSender_A,
          !error_present [*2] ##1 !error_present &&
      (prim_alert_rxtx_async_fpv.i_prim_alert_sender.state_q !=
      prim_alert_rxtx_async_fpv.i_prim_alert_sender.Idle) |->
      strong(##[1:$] (prim_alert_rxtx_async_fpv.i_prim_alert_sender.state_q ==
      prim_alert_rxtx_async_fpv.i_prim_alert_sender.Idle)),
