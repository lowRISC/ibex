...
int main(int argc, char **argv) {
  ibex_complex_system top;
  VerilatorMemUtil memutil;
  VerilatorSimCtrl &simctrl = VerilatorSimCtrl::GetInstance();
  simctrl.SetTop(&top, &top.IO_CLK, &top.IO_RST_N,
                 VerilatorSimCtrlFlags::ResetPolarityNegative);

  MemArea ram("TOP.ibex_complex_system.u_ram.u_ram.gen_generic.u_impl_generic",
              1024 * 1024 / 4, 4);
  ...
  svSetScope(svGetScopeFromName("TOP.ibex_complex_system"));

  std::cout << "\nPerformance Counters" << std::endl
            << "====================" << std::endl;
  std::cout << ibex_pcount_string(false);

  std::ofstream pcount_csv("ibex_complex_system_pcount.csv");
