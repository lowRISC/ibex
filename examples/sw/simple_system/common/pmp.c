 ...
 */
PMP_WARN_UNUSED_RESULT
static pmp_result_t pmp_csr_cfg_field_read(pmp_region_index_t region,
 ...
 */
PMP_WARN_UNUSED_RESULT
static pmp_result_t pmp_csr_cfg_field_write(pmp_region_index_t region,
                                            uint32_t field_value) {
 ...
 * @return `pmp_result_t`.
 */
pmp_result_t pmp_csr_address_write(pmp_region_index_t region,
                                   uintptr_t address) {
 ...
 */
PMP_WARN_UNUSED_RESULT
static pmp_result_t pmp_napot_address_construct(uintptr_t address,
                                                uint32_t size,
                                                uintptr_t *pmp_address_napot) {
  ...
  if (size < PMP_ADDRESS_ALIGNMENT || size < PMP_ADDRESS_MIN_ALIGNMENT_NAPOT) {
    return kPmpBadAddress;
  }

  // Check if the `size` is a Power Of Two.
  uint32_t size_mask = size - 1;
  if ((size & size_mask) != 0) {
    return kPmpNapotBadSize;
  }
...
pmp_result_t pmp_region_configure_off(pmp_region_index_t region,
                                      uintptr_t address) {
  if (region >= PMP_REGIONS_NUM) {
    return kPmpBadRegion;
  }
  ...
  pmp_result_t result = pmp_csr_address_write(region, address);
  if (result != kPmpOk) {
    return result;
  }
...
pmp_result_t pmp_region_configure_na4(pmp_region_index_t region,
                                      pmp_region_config_t config,
                                      uintptr_t address) {
  if (PMP_GRANULARITY_IBEX > 0) {
    return kPmpNa4Unavailable;
  }
...
pmp_result_t pmp_region_configure_napot(pmp_region_index_t region,
                                        pmp_region_config_t config,
  ...
  // Address registers must be written prior to the configuration registers to
  // ensure that they are not locked.
  result = pmp_csr_address_write(region, napot_address);
  if (result != kPmpOk) {
    return result;
  }
...
pmp_result_t pmp_region_configure_tor(pmp_region_index_t region_end,
                                      pmp_region_config_t config,
  ...
  // ensure that they are not locked.
  if (region_end != 0) {
    pmp_result_t result = pmp_csr_address_write(region_end - 1, address_start);
    if (result != kPmpOk) {
      return result;
    }
  }

  pmp_result_t result = pmp_csr_address_write(region_end, address_end);
  if (result != kPmpOk) {
    return result;
  }
...
pmp_result_t pmp_cfg_mode_lock_status_get(pmp_region_index_t region,
                                          pmp_region_lock_t *lock) {
  if (region >= PMP_REGIONS_NUM) {
    return kPmpBadRegion;
  }
  ...
  uint32_t field_value;
  pmp_result_t result = pmp_csr_cfg_field_read(region, &field_value);
  if (result != kPmpOk) {
    return result;
  }
