...
snippy_rtl_sim_run: $(TB-COMPILE-STAMP) scripts/run_snippy_rtl.py
...
# Snippy runs can generate standard Ibex result entries dynamically.
# This file is generated by run_snippy_rtl.py and is picked up by later
...
###############################################################################
# Run ibex RTL simulation with a Snippy-generated external payload

snippy_rtl_sim_run: scripts/run_snippy_rtl.py
	@test -f $(TB-COMPILE-STAMP) || \
	  (echo "RTL testbench is not compiled: $(TB-COMPILE-STAMP). Run GOAL=rtl_tb_compile first."; false)
	@echo Running Snippy RTL simulation for $(SNIPPY_TEST_NAME)
	$(verb)env PYTHONPATH=$(PYTHONPATH) \
	python3 scripts/run_snippy_rtl.py \
	  --dir-metadata $(METADATA-DIR) \
	  --test-name "$(SNIPPY_TEST_NAME)" \
	  --iteration "$(SNIPPY_ITERATION)" \
	  --result-json "$(SNIPPY_RESULT_JSON)" \
	  --seed "$(SNIPPY_SEED)" \
	  --linked-elf "$(SNIPPY_LINKED_ELF)" \
	  --elf-layout "$(SNIPPY_ELF_LAYOUT)" \
	  --snippy-log "$(SNIPPY_LOG)" \
	  --rtl-test "$(SNIPPY_RTL_TEST)" \
	  --timeout-s "$(SNIPPY_TIMEOUT_S)" \
	  --signature-addr "$(SNIPPY_SIGNATURE_ADDR)"
