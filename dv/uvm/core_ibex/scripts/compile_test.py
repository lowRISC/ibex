...
def get_riscvdv_compile_cmds(md: RegressionMetadata, trr: TestRunResult) -> List[str]:
    ...
    if (trr.testtype == TestType.RISCVDV and trr.is_discrete_debug_module):
        ...
        # which may be quite far away from the 'main' memory. To avoid generating a very large
        # uncompressed binary file to LOAD both segments, we probably want to use the .vmem
        # format passed to $readmemh to initialize the RTL-side memory model. However, as the
        # cosim memory model is initialized over the DPI interface, and may be comprised of
        # multiple 'memories' rather than a single contiguous space, we can't simply rely on
        # $readmemh to automatically partition a single file across multiple arrays. Hence
        # for it's initialization, generate raw .bin files which will be loaded via the base
        ...
        vmem_main_cmd = [env.get('RISCV_OBJCOPY'), "-O", "verilog", *remove_sections, f"{trr.objectfile}", f"{trr.vmem_main}"]

        # Generate binary-formatted outputs (.bin)

        trr.binary_main = trr.dir_test / "test.main.bin"
        trr.binary_dm = trr.dir_test / "test.dm.bin"
        bin_dm_cmd = [env.get('RISCV_OBJCOPY'), "-O", "binary", *only_sections, f"{trr.objectfile}", f"{trr.binary_dm}"]
        bin_main_cmd = [env.get('RISCV_OBJCOPY'), "-O", "binary", *remove_sections, f"{trr.objectfile}", f"{trr.binary_main}"]
