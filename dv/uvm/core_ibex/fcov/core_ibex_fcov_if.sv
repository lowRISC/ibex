...
interface core_ibex_fcov_if import ibex_pkg::*; (
  ...
  // ibex_decoder.sv (OPCODE_JALR case) routes JALR through cheriot_ex as a
  // single-cycle CJALR once cheriot_enable_i is asserted, and never asserts
  // jump_in_dec_o in that mode (see the decoder's `else` branch, which is
  // the only place jump_in_dec_o is set for JALR). id_instr_category still
  // (correctly) classifies the opcode as InstrCategoryJump regardless of
  // mode, so the correctness check here must accept either path rather than
  // only the plain-RISC-V one.
  `ASSERT(InstrCategoryJumpCorrect,
      id_instr_category == InstrCategoryJump |->
      id_stage_i.jump_in_dec || id_stage_i.instr_is_cheriot_id_o)

  // ibex_decoder.sv's OPCODE_LOAD/OPCODE_STORE cases route CLC/CSC (capability
  // load/store, instr[13:12]==2'b11 under cheriot_enable_i) through cheriot_ex
  // via cheriot_data_req_o instead of the normal data_req_o/lsu_req_dec path
  // (data_req_o is explicitly forced to 0 for CLOAD_CAP/CSTORE_CAP -- "req
  // generated by cheriot_ex"). id_instr_category still (correctly) classifies
  // the opcode as InstrCategoryLoad/Store regardless of mode, so the
  // correctness check here must accept either path, mirroring
  // InstrCategoryJumpCorrect's CJALR accommodation above.
  `ASSERT(InstrCategoryLoadCorrect,
      id_instr_category == InstrCategoryLoad |->
      (id_stage_i.lsu_req_dec && !id_stage_i.lsu_we) || id_stage_i.instr_is_cheriot_id_o)
