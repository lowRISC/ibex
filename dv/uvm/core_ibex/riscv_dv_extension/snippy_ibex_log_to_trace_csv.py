...
INSTR_RE = \
    re.compile(r"^\s*(?P<time>\d+)\s+(?P<cycle>\d+)\s+(?P<pc>[0-9a-f]+)\s+"
               r"(?P<bin>[0-9a-f]+)\s+(?P<instr>\S+)(?:\s+(?P<ops>.*?))?\s*$")
RD_RE = re.compile(r"(x(?P<rd>[1-9]\d*)=0x(?P<rd_val>[0-9a-f]+))")
ADDR_RE = re.compile(r"(?P<rd>[a-z0-9]+?),"
                     r"(?P<imm>[\-0-9]+?)"
                     r"\((?P<rs1>[a-z0-9]+)\)")

def strip_wb_from_ops(ops: str) -> str:
    """Strip writeback parts from ops.
    Supports formats: 'x1=0x...', 'x1:0x...' and chains 'x0:0x... x12:0x...'
    """
    if not ops:
        return ""
    # We cut off everything that starts with the first writeback token:
    # space + x<digits> + (= or :) + 0x...
    ops = re.split(r"\s+x\d+(?:=|:)\s*0x[0-9a-fA-F]+", ops, maxsplit=1)[0]
    return ops.strip()
...
def _process_ibex_sim_log_fd(log_fd, csv_fd, full_trace=True):
    ...
    for line in log_fd:
        ...
        trace_entry.instr_str = m.group("instr") + (" " + m.group("ops") if m.group("ops") else "")
        trace_entry.instr = m.group("instr")
        trace_entry.pc = m.group("pc")
        trace_entry.binary = m.group("bin")

        if full_trace:
            ops = m.group("ops") if m.group("ops") else ""
            ops = strip_wb_from_ops(ops)
            expand_trace_entry(trace_entry, ops)
...
def expand_trace_entry(trace, operands):
    ...
    '''
    # --- minimal safety: allow instructions without operands ---
    if operands is None:
        operands = ""
    operands = operands.strip()

    # Even if operands are empty, still run pseudo-instr conversion
    # (some tools expect instr normalization regardless of operands).
    if operands:
    ...
    trace.operand = convert_operands_to_abi(operands) if operands else ""
