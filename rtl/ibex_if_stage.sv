...
module ibex_if_stage import ibex_pkg::*; import ibex_cheriot_pkg::*; #(
...
`endif
  ...
  if (DummyInstructions) begin : gen_dummy_instr
    ...
    // A dummy inserted while the compressed decoder presents an expansion micro-op must not advance
    // the Zcmp expander: cm_state_q always holds, and the rlist/sp_offset position holds while a
    // sequence is in flight. In CmIdle those two are a combinational pre-load from instr_i that is
    // recomputed once the dummy has passed, so they are not checked there. The antecedent requires
    // a genuine accept (valid, ready, no redirect) because a flush also reloads the expander.
...
`ifdef INC_ASSERT
    ...
    // pc_set_i takes precedence over branch prediction
